...
class InMemoryRESTsMRIDataset(RESTsMRIDataset):
    def __init__(
        ...
        super(InMemoryRESTsMRIDataset, self).__init__(
            metadata_path, data_dir, splits, normalize, dtype
        )
        if self._cache_exists():
            self.data = self._load_cache()
        else:
            self.data = self._preprocess_images(metadata_path, data_dir, splits)
            self._save_cache()
    ...
    def _preprocess_images(self, metadata_path, data_dir, splits):
        metadata = pd.read_csv(metadata_path)
        data = np.zeros(
...
class LazyRESTsMRIDataset(RESTsMRIDataset):
    def __init__(
        ...
        self._load_ids(metadata_path)

        if not self._cache_exists():
            self._create_cache(metadata_path, data_dir, splits)
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(data, dtype=NP_TO_TORCH_DTYPES[self.dtype])

    def _load_ids(self, metadata_path):
        metadata = pd.read_csv(metadata_path)
        self.ids = metadata.subID

    def _create_cache(self, metadata_path, data_dir, splits):
        if not os.path.exists(self.cache_path):
            os.makedirs(self.cache_path)
